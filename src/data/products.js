...
if (!storedProducts) {
  localStorage.setItem('products_js', JSON.stringify(defaultProducts));
}

// Combine default products with stored products
const storedProductsArray = JSON.parse(localStorage.getItem('products_js')) || [];
export const products = [...defaultProducts, ...storedProductsArray.filter(p => 
  !defaultProducts.some(dp => dp.id === p.id)
)];

// Keep localStorage in sync
localStorage.setItem('products_js', JSON.stringify(products));

export const getFeaturedProducts = () => {
  const currentProducts = JSON.parse(localStorage.getItem('products_js')) || products;
  return currentProducts.filter(product => product.isFeatured);
};
